...
other opt-in      ███              3   Function source · Polaris manifest · 1 candidate
```
...
`functions-no-unavailable-runtime-apis` on **JS Function source only**. `s-known-components` on App Home source, beside the recipe. 🧪 `no-session-or-token-logging` is a candidate, off by default.

```tsx
<s-stak gap="base" />                           // ❌ unknown element
<s-button varient="primary">Save</s-button>     // ❌ unknown attribute
<s-button variant="primary" onClick={save}>Save</s-button> // ✅ declared attribute and event
```

**`s-known-components` reads its vocabulary from your project, never from this package:** the Custom Elements Manifest named by the `customElements` field of the nearest installed `@shopify/polaris-types` (1.0.7 and later). It follows whichever major you install. **No package, or no `customElements` field: the rule stays silent.** An explicit `manifestPath` that can't be read, or a manifest without elements, reports once per file.
...
| `s-known-components`                    | opt-in, App Home source                                                               | `s-*` elements the manifest doesn't declare; attributes that aren't a declared attribute, public field or `on<event>` handler. Always allowed: `key`, `ref`, `slot`, `children`, `id`, `data-*`. Children of allowed elements (App Bridge nav links) keep their own contract        | `manifestPath` (discovered), `allowElements` (`["s-app-nav", "s-app-window"]`, replaced), `allowAttributes` (`[]`)                                |
...
| 2026-09-20 · server & API     | [Admin GraphQL reference](https://shopify.dev/docs/api/admin-graphql/latest), [idempotency changelog](https://shopify.dev/changelog/making-idempotency-mandatory-for-inventory-adjustments-and-refund-mutations), [idempotency guide](https://shopify.dev/docs/api/usage/implementing-idempotency), control flow of [`shopify-app-js`](https://github.com/Shopify/shopify-app-js), [React Router app template](https://github.com/Shopify/shopify-app-template-react-router), [billing API reference](https://shopify.dev/docs/api/shopify-app-react-router/latest/apis/billing), [webhook HTTPS delivery](https://shopify.dev/docs/apps/build/webhooks/subscribe/https), [JavaScript for Functions](https://shopify.dev/docs/apps/build/functions/programming-languages/javascript-for-functions), [Function input/output limits](https://shopify.dev/docs/apps/build/functions/input-output), [protected customer data](https://shopify.dev/docs/apps/launch/protected-customer-data), [Script Tag deprecation changelog](https://shopify.dev/changelog/online-store-script-tags-deprecation) |
| 2026-09-24 · Polaris manifest | [App Home web components](https://shopify.dev/docs/api/app-home/latest/web-components), the `customElements` field and manifest schema of the [Custom Elements Manifest](https://github.com/webcomponents/custom-elements-manifest) format, as shipped by `@shopify/polaris-types` 1.1.0 and 2.0.0-rc.0 (same tags and attributes)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `s-known-components`                    | Disallow `s-*` elements and attributes that the project's installed `@shopify/polaris-types` Custom Elements Manifest does not declare; silent when no manifest resolves. Options `manifestPath`, `allowElements` and `allowAttributes`. |
...
- https://github.com/webcomponents/custom-elements-manifest (inspiration; independently implemented)
...
- https://shopify.dev/docs/api/app-home/latest/web-components (inspiration; independently implemented)
