...
async function lintCode(ruleName: string, code: string, options: LintCodeOptions = {}): Promise<LintResult> {
  const directory = await mkdtemp(path.join(tmpdir(), "oxlint-plugin-type-evidence-"));
  ...
  await writeFile(
    configPath,
    JSON.stringify(
      { categories: { correctness: "off" }, jsPlugins: [`file:///${pluginPath}`], rules: { [ruleName]: ruleConfig } },
      null,
      2,
    ),
  );
